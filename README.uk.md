...
Оригінальна задача — [`breef.md`](breef.md) (лише українською). Коротко, що там було:
...
Реалізація пішла далі брифу: exponential decay, roster weight ×4, seed prior, 6 Swiss-кошиків, seed guard для 0-3, two-phase CLI (`scrape` / `analyze`). Деталі — [`MODEL.uk.md`](MODEL.uk.md).
...
| Файл | Що там |
| --- | --- |
| [`breef.md`](breef.md) | Оригінальний бриф задачі (UK) |
...
| `stage{N}_report.md` | Output — таблиця ймовірностей + recommended Pick'Em |

---

## Stage report — що всередині

Файл `stage1_report.md`, `stage2_report.md` … генерується в **корені проєкту** командою `analyze`. Комітиться в git, щоб можна було побачити результат без Liquipedia. Після нового `analyze` — оновлюй і коміть, якщо хочеш поділитись свіжим прогнозом.
...
Детально: **[`MODEL.uk.md`](MODEL.uk.md)**.
